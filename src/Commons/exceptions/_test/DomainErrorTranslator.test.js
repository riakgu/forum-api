...
describe('DomainErrorTranslator', () => {
    it('should translate error correctly', () => {
        ...
        expect(DomainErrorTranslator.translate(new Error('NEW_THREAD_COMMENT.CONTENT_CANNOT_BE_EMPTY')))
        ...
        expect(DomainErrorTranslator.translate(new Error('THREAD_DETAIL.NOT_CONTAIN_NEEDED_PROPERTY')))
            .toStrictEqual(new InvariantError('tidak dapat membuat thread detail karena properti yang dibutuhkan tidak ada'));
        expect(DomainErrorTranslator.translate(new Error('THREAD_DETAIL.NOT_MEET_DATA_TYPE_SPECIFICATION')))
            .toStrictEqual(new InvariantError('tidak dapat membuat thread detail karena tipe data tidak sesuai'));
        expect(DomainErrorTranslator.translate(new Error('THREAD_COMMENT.NOT_CONTAIN_NEEDED_PROPERTY')))
            .toStrictEqual(new InvariantError('tidak dapat membuat thread comment karena properti yang dibutuhkan tidak ada'));
        expect(DomainErrorTranslator.translate(new Error('THREAD_COMMENT.NOT_MEET_DATA_TYPE_SPECIFICATION')))
            .toStrictEqual(new InvariantError('tidak dapat membuat thread comment karena tipe data tidak sesuai'));
        expect(DomainErrorTranslator.translate(new Error('NEW_THREAD_COMMENT_REPLY.NOT_CONTAIN_NEEDED_PROPERTY')))
            .toStrictEqual(new InvariantError('harus mengirimkan content'));
        expect(DomainErrorTranslator.translate(new Error('NEW_THREAD_COMMENT_REPLY.NOT_MEET_DATA_TYPE_SPECIFICATION')))
            .toStrictEqual(new InvariantError('content harus string'));
        expect(DomainErrorTranslator.translate(new Error('NEW_THREAD_COMMENT_REPLY.CONTENT_CANNOT_BE_EMPTY')))
            .toStrictEqual(new InvariantError('content tidak boleh kosong'));
        expect(DomainErrorTranslator.translate(new Error('THREAD_COMMENT_REPLY.NOT_CONTAIN_NEEDED_PROPERTY')))
            .toStrictEqual(new InvariantError('tidak dapat membuat thread comment karena properti yang dibutuhkan tidak ada'));
        expect(DomainErrorTranslator.translate(new Error('THREAD_COMMENT_REPLY.NOT_MEET_DATA_TYPE_SPECIFICATION')))
            .toStrictEqual(new InvariantError('tidak dapat membuat thread comment karena tipe data tidak sesuai'));
    });
